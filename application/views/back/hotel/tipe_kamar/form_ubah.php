    ...
    <section class="content">
       <div class="container-fluid">
          <div class="card">
             ...
             <div class="card-body">
                <form action="<?= base_url('admin/tipe_ubah_aksi') ?>" method="post" enctype="multipart/form-data">
                   <div class="row">
                      <div class="col-md-6">
                         ...
                         <!-- <div class="form-group">
                            <div class="row">
                               ...
                               <div class="col-md-9">
                                  <input type="number" name="kids_occupancy" required class="form-control" value="<?php echo set_value('kids_occupancy', $tipe_kamar->kids_occupancy) ?>" id="kids_occupancy" />
                            ...
                            <div class="row">
                               ...
                               <div class="col-md-9">
                                  <?php foreach ($fasilitas as $am) { ?>
                                     <b><?= $am->nama ?> </b> &nbsp;&nbsp; <input type="checkbox" name="fasilitas[<?= $am->id_fasilitas ?>]" value="1" <?= (in_array($am->id_fasilitas, $tipe_fasilitas)) ? 'checked="checked"' : ''; ?> /> &nbsp;&nbsp;
                                  <?php } ?>
                         ...
                         <div class="form-group">
                            <div class="row">
                               ...
                               <div class="col-md-9">
                                  <input type="number" name="tarif_dasar" required class="form-control" value="<?php echo set_value('tarif_dasar', $tipe_kamar->tarif_dasar) ?>" id="tarif_dasar" />
                         ...
                         <div class="input_fields_wrap">
                            ...
                            <?php if (empty($image)) { ?>
                               <div class="row ">
                                  ...
                                  <div style="padding-top:20px;"><button class="add_field_button btn btn-sm btn-success shadow"><i class="fas fa-plus-circle"></i>Tambah</button>
                                  </div>
                               </div>
                            <?php } ?>
                              ...
                              foreach ($image as $img) { ?>
                               <div class="row ">
                                  <div class="col-md-6 mb-1">
                                     <label>Gambar</label>
                                     <input type="file" name="image[]" class="form-control imgchange" id="<?= $img->id_tipe_gambar ?>" onchange="readURL(this,urblah<?= $i ?>);" />
                                  </div>
                                  <div class="col-md-4 mb-1 urblah<?php echo $i ?>">
                                     <img class="blah " src="<?php echo base_url('assets/img/kamar/small/' . $img->image) ?>" alt="your image" id="urblah<?= $i ?>" width="100" height="80" />
                                  </div>
                                  <?php if ($i == 0) { ?>
                                     <div style="padding-top:20px;"><button class="add_field_button btn btn-sm btn-success shadow">Tambah</button></div>
                                  <?php } else { ?>
                                     <div class="col-md-2">
                                        <div style="padding-top:20px;"><a href="#" id="<?= $img->id_tipe_gambar ?>" class="remove_fieldedit btn btn-sm btn-danger shadow">Hapus</a></div>
                                     </div>
                                  <?php } ?>
                               </div>
                            <?php $i++;
 ...
 <script>
    ...
    $(document).ready(function() {
       ...
       $(add_button).click(function(e) { //on add input button click
          ...
          if (x < max_fields) { //max input box allowed
             ...
             $(wrapper).append('<div class="row"><div class="col-md-6"><label>image</label><input type="file" name="image[]" class="form-control" onchange="readURL(this,blah' + x + ');"  /></div><div class="col-md-4  blah' + x + '"  ><img class="blah hide" src="#" alt="your image" id="blah' + x + '" /></div><div class="col-md-2"><div style="padding-top:20px;"><a href="#" class="remove_field btn btn-danger shadow">Hapus</a></button></div></div>'); //add input box
    ...
    <?php if ($tipe_kamar->id_tipe_kamar) { ?>
       $(document).on("click", ".remove_fieldedit", function(e) { //user click on remove text
          e.preventDefault();
          var id = event.target.id;
          if (id) {

             if (window.confirm("Apakah yakin akan menghapus foto ?")) {
                call_loader();
                $.ajax({
                   url: '<?php echo site_url('backend/hotel/updateimg') ?>',
                   type: 'POST',
                   data: {
                      id: id
                   },
    ...
    $(document).on("change", ".imgchange", function(e) { //user click on remove text
       ...
       if (id) {
          call_loader();
          $.ajax({
             url: '<?php echo site_url('backend/hotel/edit_image'); ?>/' + id,
             type: 'POST',
             xhr: function() {
